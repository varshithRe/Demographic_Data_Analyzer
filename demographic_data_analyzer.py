...
def calculate_demographic_data(print_data=True):
    ...
    num_high_each_country = df[df['salary']=='>50K']['native-country'].value_counts()
    num_country = df['native-country'].value_counts()
    percentageof = (num_high_each_country/num_country)*100
    highest_earning_country = percentageof.idxmax()
    highest_earning_country_percentage = round( percentageof.max(),1)

    # Identify the most popular occupation for those who earn >50K in India.
    high_salary_ind = df[(df['salary'] == ">50K") & (df['native-country'] == "India")]['occupation'].value_counts()
    top_IN_occupation = high_salary_ind.idxmax()
